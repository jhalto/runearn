...
class AccountRemoteDataSource {
  const AccountRemoteDataSource(this.firestore, this.auth);
  ...
  CollectionReference<Map<String, dynamic>> get _collection {
    ...
    return firestore.collection('users').doc(user.uid).collection('accounts');
  }
  ...
  Future<List<Map<String, dynamic>>> getTransfers() async {
    final snapshot = await _transfers.get();
    return snapshot.docs
        .map((document) => {...document.data(), 'id': document.id})
        .toList(growable: false);
  }
